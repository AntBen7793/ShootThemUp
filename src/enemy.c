...
void init_enemy(Enemy **enemies, int *nb_enemy, int x, int y, int type)
{
    ...
    {
    case 2:
        enemy.sprite = MLV_load_image("./img/kamikaze.png");
        enemy.speed = 5;
        break;
    case 5:
        enemy.sprite = MLV_load_image("./img/sniper.png");
        enemy.speed = 1;
        break;
    case 6:
        enemy.sprite = MLV_load_image("./img/tank.png");
        enemy.speed = 1;
        break;
    default:
...
void update_enemy(Enemy **enemies, int *nb_enemy, MLV_Image **explosion_images, MLV_Sound **sound, Missile_enemy **missiles_enemy, Stats *stats, int *nb_missile_enemy, double *effect_volume)
{
    for (int i = 0; i < *nb_enemy; i++)
    {
        ...
        if (enemy->life <= 0)
        {
            ...
            if (enemy->explosion_state < 14 && enemy->explosion_state % 2 == 0)
            {
               
                enemy->boom = explosion_images[enemy->explosion_state - (enemy->explosion_state / 2)];
                MLV_resize_image(enemy->boom, enemy->width, enemy->width);
            }
            if (enemy->explosion_state == 1)
            {
                MLV_play_sound((*sound), *effect_volume);
            }
            if(enemy->explosion_state == 10){
                enemy->sprite = NULL;
            }
            if (enemy->explosion_state > 15)
            {
...
void shoot_enemy(Enemy *enemy, Missile_enemy **missiles_enemy, int *nb_missile_enemy)
{
    ...
    case 5:
        if (enemy->cooldown == 80)
        {
            // init_missile_enemy(missiles_enemy, nb_missile_enemy, (enemy->x)+(enemy->width/2), enemy->y);
            init_missile_enemy(missiles_enemy, nb_missile_enemy, (enemy->x) + (enemy->width / 2), enemy->y + 10);
            enemy->cooldown = 0;
        }
        else if (enemy->cooldown == 75)
        {

            init_missile_enemy(missiles_enemy, nb_missile_enemy, (enemy->x) + (enemy->width / 2), enemy->y + 10);
            enemy->cooldown++;
        }
        else if (enemy->cooldown == 70)
        {
            init_missile_enemy(missiles_enemy, nb_missile_enemy, (enemy->x) + (enemy->width / 2), enemy->y + 10);
            enemy->cooldown++;
        }
    ...
    case 6:
        if (enemy->cooldown >= 80)
        {
            ...
            init_missile_enemy(missiles_enemy, nb_missile_enemy, (enemy->x) + 10, enemy->y);
            init_missile_enemy(missiles_enemy, nb_missile_enemy, (enemy->x) + ((enemy->width) - 20), enemy->y);
            init_missile_enemy(missiles_enemy, nb_missile_enemy, (enemy->x) + ((enemy->width) - 40), enemy->y);
            enemy->cooldown = 0;
        }
        else
        {
...
void draw_enemy(Enemy *enemy)
{
    ...
    if(enemy->boom){
         MLV_draw_image((*enemy).boom, (*enemy).x, (*enemy).y);
    }
